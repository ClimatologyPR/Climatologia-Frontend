                            ...
                            <v-row class="pl-3 pr-3">
                                ...
                                <v-col v-if="selectedDateType === 'Día'" cols="5" >
                                    ...
                                    >
                                        ...
                                        <v-date-picker locale="es-ES"  v-model="date" color="#82ada9" @input="validate('single')" no-title  :show-current="false" scrollable min="2000-08-15" max="2019-03-20">
                                        </v-date-picker>
                                    </v-dialog>
                                </v-col>
                            ...
                            <v-tooltip top>
                                <template v-slot:activator="{ on }">
                                    <v-btn
                                        ...
                                        :disabled="disable"
                                        @click="setButtonPressed('btn1'),  fetchStations('prcp', (rangeDate?   date[0] :  date ), (rangeDate? date[1] :   date ))"
                                    >
                            ...
                            <v-tooltip top>
                                <template v-slot:activator="{ on }">
                                    <v-btn
                                        ...
                                        :disabled="disable"
                                        @click="setButtonPressed('btn2'),  fetchStations('tmax', (selectedDateType === 'Rango'?   date[0] :   date ), (rangeDate? date[1] :   date ))"
                                    >
                            ...
                            <v-tooltip top>
                                <template v-slot:activator="{ on }">
                                    <v-btn
                                        ...
                                        :disabled="disable"
                                        @click="setButtonPressed('btn3'),  fetchStations('tmin', (rangeDate?  date[0] :  date ), (rangeDate? date[1] :  date ))"
                                    >
                                        <v-icon>mdi-thermometer-lines</v-icon>
                                    </v-btn>
                                </template>
...
export default {
    data() { return {
         buttons: {btn1: {class:'ma-2 btn1'}, btn2: {class: 'ma-2 btn2'}, btn3: {class: 'ma-2 btn3'}},
    ...
    validate: function (picker) {
            if (this.date[0] && this.date[1] && this.date.length ===2 && picker === 'range') {   
    ...
    setButtonUnpressed: function () {    
        if (this.currentPressButton){
            this.buttons[this.currentPressButton].class = `ma-2 ${this.currentPressButton}`
        }
    },
